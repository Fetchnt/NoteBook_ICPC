...
\begin{itemize}
    ...
    \item \textbf{Consulta por rango (Query):} $\mathcal{O}(\log N)$
\end{itemize}
\textbf{Complejidad de Espacio:} $\mathcal{O}(N)$ (usa $2S$ nodos, donde $S$ es la siguiente potencia de 2). \\
Estructura iterativa para consultas asociativas en intervalos dinámicos. Reduce memoria frente a la versión recursiva de $4N$ y evita llamadas recursivas. La consulta usa el intervalo semiabierto $[l,r)$.
\begin{lstlisting}
// Versión iterativa: usa 2*size nodos y elimina el costo de recursión.
struct SegTree {
    int n;
    int size;
    vector<int> tree;
    ...
    int query(int l, int r) const { // [l, r), 0-indexado
        int ans = 0;
...
Permite modificaciones masivas con *lazy propagation*. La función \texttt{apply} actualiza directamente un nodo y su etiqueta, reduciendo operaciones repetidas de suma y propagación.
\begin{lstlisting}
struct SegTreeLazy {
    int n; vector<ll> tree, lazy;
    SegTreeLazy(int n) : n(n), tree(4 * n, 0), lazy(4 * n, 0) {}

    inline void apply(int node, int start, int end, ll val) {
        tree[node] += (end - start + 1LL) * val;
        lazy[node] += val;
    }
    ...
    inline void push(int node, int start, int end) {
        ...
        int mid = (start + end) >> 1;
        ll val = lazy[node];
        apply(node << 1, start, mid, val);
        apply(node << 1 | 1, mid + 1, end, val);
        lazy[node] = 0;
    }
    ...
    ll queryRange(int node, int start, int end, int l, int r) {
        ...
        push(node, start, end);
        int mid = (start + end) >> 1;
        return queryRange(node << 1, start, mid, l, r) +
               queryRange(node << 1 | 1, mid + 1, end, l, r);
    }
};
\end{lstlisting}
...
struct MergeSortTree {
    ...
    void build(const vector<int>& arr, int node, int start, int end) {
        ...
        // Reserva exactamente el tamaño final para evitar realocaciones.
...
\end{itemize}
\textbf{Complejidad de Espacio:} $\mathcal{O}(N \log N)$, sin arreglo auxiliar de logs. \\
...
struct SparseTable {
    ...
    int n;
    SparseTable(const vector<int>& arr) : n((int)arr.size()) {
        int K = (n == 0 ? 0 : 32 - __builtin_clz(n));
    ...
    // Mínimo en el rango [l, r] (0-indexado, inclusivo).
    // __builtin_clz evita almacenar un arreglo logs de tamaño N.
...
vector<ll> dijkstra(int src, int n,
    ...
    while (!pq.empty()) {
        auto [d, u] = pq.top();
        pq.pop();
        if (d != dist[u]) continue; // descarta entradas obsoletas
...
\textbf{Complejidad:} $\mathcal{O}(V \cdot E)$ en el peor caso; puede terminar antes si no hay más relajaciones.
\begin{lstlisting}
...
pair<vector<ll>, bool> bellmanFord(int src, int n, vector<Edge>& edges) {
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        if (!changed) break; // ya convergió; evita pasadas innecesarias
    }
...
void floydWarshall(vector<vector<ll>>& dist, int n) {
    ...
    for (int k = 0; k < n; k++) {
        for (int i = 0; i < n; i++) {
            if (dist[i][k] == INF_LL) continue;
            const ll dik = dist[i][k];
            for (int j = 0; j < n; j++) {
                if (dist[k][j] == INF_LL) continue;
...
ll kruskal(int n, vector<EdgeW>& edges) {
    ...
    for (const auto& e : edges) {
        if (dsu.find(e.u) != dsu.find(e.v)) {
            dsu.unite(e.u, e.v);
            costoTotal += e.w;
            if (++aristasUsadas == n - 1) break; // MST completa
        }
    }
...
double closest_pair_rec(vector<Point>& pts, int l, int r) {
    if (r - l <= 3) {
        ...
        for (int i = l; i < r; i++)
            for (int j = i + 1; j < r; j++)
                best2 = min(best2, dist2(pts[i], pts[j]));
    ...
    double d2 = min(closest_pair_rec(pts, l, mid),
    ...
    for (int i = l; i < r; i++) {
        ...
        if (dx * dx < d2) strip.push_back(pts[i]);
    }
...
double closest_pair(vector<Point> pts) {
    ...
    return sqrt(closest_pair_rec(pts, 0, (int)pts.size()));
}
\end{lstlisting}
...
// Números de Catalan. La recurrencia cuadrática es correcta, pero innecesaria
// cuando n < MOD: C_i = C_{i-1} * (4i-2) / (i+1).
vector<ll> tabla_catalan(int n) {
    vector<ll> C(n + 1, 0);
    C[0] = 1;
    for (int i = 1; i <= n; i++) {
        // Requiere i + 1 < MOD para usar inv_mod.
        C[i] = C[i - 1] * (4LL * i - 2) % MOD * inv_mod(i + 1) % MOD;
    }
    return C;
}
...
% =========================================================================
% CAPITULO EXTRA: APLICACIONES SENCILLAS
% =========================================================================
\chapter{Aplicaciones Sencillas de Cada Código}
Esta sección muestra cómo pasar de la plantilla de la biblioteca a un problema
pequeño. La idea no es memorizar el ejemplo exacto, sino identificar qué datos
entran, qué función se llama y qué información devuelve. Cada bloque de la
biblioteca tiene aquí una aplicación correspondiente.

\section{Aplicación 1: Configuración Inicial (Template C++)}
Usa esta plantilla cuando empieces un problema de concurso: activa I/O rápido y deja un punto claro para la solución.
...
\section{Aplicación 2: Lectura Eficiente (Fast I/O)}
Úsalo cuando el juez entrega cientos de miles o millones de datos.
...
\section{Aplicación 3: Lectura Eficiente (Fast I/O) (ejemplo 2)}
Lee datos de entrada de forma cómoda y rápida.
...
\section{Aplicación 4: Búsqueda Binaria (Algoritmo Floor)}
Ejemplo: encontrar el precio más alto que no supera un presupuesto.

\begin{lstlisting}
vector<int> precios = {10, 20, 35, 50, 80};
int presupuesto = 42;
int pos = binary_search_floor(precios, presupuesto);
// pos = 2, porque precios[2] = 35
cout << precios[pos];
\end{lstlisting}

\section{Aplicación 5: Fenwick Tree (Binary Indexed Tree)}
Ejemplo: guardar ventas diarias, actualizar un día y consultar la suma de un intervalo.

\begin{lstlisting}
BIT bit(5);
for (int i = 1; i <= 5; i++) bit.update(i, i * 10);

bit.update(3, 25);          // sumar 25 al dia 3
cout << bit.query(2, 4);    // suma de los dias 2..4
\end{lstlisting}

\section{Aplicación 6: Segment Tree (Consultas por Rangos)}
Ejemplo: cambiar el valor de un elemento y preguntar la suma de cualquier intervalo.
...
st.update(1, 20);
st.update(2, 30);
cout << st.query(0, 3);     // 10 + 20 + 30
\end{lstlisting}

\section{Aplicación 7: Segment Tree con Lazy Propagation}
Ejemplo: aumentar todos los salarios de un rango de empleados sin actualizarlos uno por uno.

\begin{lstlisting}
SegTreeLazy st(5);

// Sumar 100 a los empleados 1..3
st.updateRange(1, 0, 4, 1, 3, 100);

cout << st.queryRange(1, 0, 4, 1, 3);
\end{lstlisting}

\section{Aplicación 8: Merge Sort Tree}
Ejemplo: contar cuántos valores menores que K aparecen dentro de un intervalo.

\begin{lstlisting}
vector<int> a = {8, 2, 7, 3, 5};
MergeSortTree mst(a.size());
mst.build(a, 1, 0, a.size() - 1);

// ¿Cuantos valores menores que 6 hay entre indices 1 y 4?
cout << mst.query(1, 0, 4, 1, 4, 6);
\end{lstlisting}

\section{Aplicación 9: Sparse Table (RMQ Estático)}
Ejemplo: consultar rápidamente la temperatura mínima entre dos días cuando el arreglo no cambia.

\begin{lstlisting}
vector<int> temperatura = {30, 25, 28, 20, 35};
SparseTable st(temperatura);

cout << st.query(1, 4);      // minimo entre dias 1 y 4
\end{lstlisting}

\section{Aplicación 10: DSU con Unión por Tamaño (Rollback-Ready)}
Ejemplo: unir ciudades y después deshacer una unión en un algoritmo offline.
...
cout << (dsu.find(1) == dsu.find(3)); // 1

dsu.rollback(); // deshace la ultima union
\end{lstlisting}

\section{Aplicación 11: Trie (Árbol de Prefijos)}
Ejemplo: guardar números y preguntar cuál produce el XOR máximo con un número dado.
...
\section{Aplicación 12: Conceptos y Uso Básico en STL}
Ejemplo sencillo de aplicación del bloque de código.

\begin{lstlisting}
// Preparar los datos del problema
// Ejecutar el algoritmo de esta seccion
// Mostrar el resultado
\end{lstlisting}

\section{Aplicación 13: Conceptos y Uso Básico en STL (ejemplo 2)}
Ejemplo directo de uso de cnt\_le sobre datos pequeños para ver qué resultado produce.

\begin{lstlisting}
// Datos de ejemplo
// cnt_le(...);
cout << "Resultado obtenido por cnt_le";
\end{lstlisting}

\section{Aplicación 14: Búsqueda con Comparador Personalizado}
Busca un elemento dentro de una estructura ordenada usando el campo que nos interesa.

\begin{lstlisting}
auto it = lower_bound(gente.begin(), gente.end(), 20,
    [](const Persona& p, int x) { return p.edad < x; });
\end{lstlisting}

\section{Aplicación 15: lower\_bound / upper\_bound en set y multiset}
Consulta directamente el siguiente o anterior elemento en un conjunto ordenado.

\begin{lstlisting}
multiset<int> s = {1, 3, 3, 5, 7};
auto it = s.lower_bound(4);
cout << *it;
\end{lstlisting}

\section{Aplicación 16: Búsqueda Binaria Genérica sobre Predicado (Adaptativa)}
Ejemplo: encontrar la capacidad mínima de un camión que permite transportar todos los paquetes.

\begin{lstlisting}
auto check = [&](long long capacidad) {
    // Simular el reparto y devolver true si alcanza.
    return capacidad >= 100;
};

long long respuesta =
    binary_search_first_true(1LL, 1000LL, check);
\end{lstlisting}

\section{Aplicación 17: Implementación Manual (Referencia Interna)}
Permite entender o reutilizar la búsqueda binaria sin depender de la STL.

\begin{lstlisting}
vector<int> a = {1, 3, 5, 8};
int pos = my_lower_bound(a, 5);
cout << pos;
\end{lstlisting}

\section{Aplicación 18: Plantilla Universal de Grafo}
Ejemplo directo de uso de leerGrafoNoDirigido sobre datos pequeños para ver qué resultado produce.

\begin{lstlisting}
// Datos de ejemplo
// leerGrafoNoDirigido(...);
cout << "Resultado obtenido por leerGrafoNoDirigido";
\end{lstlisting}

\section{Aplicación 19: BFS (Búsqueda en Anchura)}
Ejemplo: encontrar el número mínimo de calles que hay que recorrer desde una ciudad.

\begin{lstlisting}
vector<vector<int>> adj(n + 1);
bfs(1, n, adj);

cout << dist_[destino];     // cantidad minima de aristas
vector<int> camino = reconstruirCamino(destino);
\end{lstlisting}

\section{Aplicación 20: BFS Multi-Fuente}
Ejemplo: varias estaciones de bomberos atienden simultáneamente a las ciudades más cercanas.

\begin{lstlisting}
vector<int> estaciones = {1, 5, 8};
vector<int> dist = bfsMultiFuente(n, adj, estaciones);

cout << dist[10];            // distancia a la estacion mas cercana
\end{lstlisting}

\section{Aplicación 21: BFS 0/1 (Deque)}
Ejemplo: moverse por un mapa donde algunas transiciones cuestan 0 y otras cuestan 1.

\begin{lstlisting}
vector<vector<pair<int,int>>> adjW(n + 1);
vector<int> dist = bfs01(1, n, adjW);

cout << dist[n];
\end{lstlisting}

\section{Aplicación 22: DFS (Búsqueda en Profundidad)}
Ejemplo: recorrer una red y visitar todos los nodos alcanzables desde una ciudad.
...
\section{Aplicación 23: DFS Iterativo (evita stack overflow)}
Úsalo cuando el grafo puede tener cientos de miles de nodos y una DFS recursiva podría agotar la pila.

\begin{lstlisting}
dfsIterativo(1, n, adj);
// Todos los nodos alcanzables desde 1 quedaron marcados.
\end{lstlisting}

\section{Aplicación 24: Componentes Conexas y Grafos Bipartitos}
Ejemplo: contar grupos desconectados y comprobar si se pueden dividir en dos equipos.

\begin{lstlisting}
vector<int> compId;
int grupos = contarComponentes(n, adj, compId);

bool dosEquipos = esBipartito(n, adj);
\end{lstlisting}

\section{Aplicación 25: Grafo NO Dirigido}
Ejemplo: detectar si una red de carreteras contiene algún ciclo.

\begin{lstlisting}
bool hayCiclo = tieneCicloNoDirigido(n, adj);

cout << (hayCiclo ? "SI" : "NO");
\end{lstlisting}

\section{Aplicación 26: Grafo Dirigido (3 colores)}
Ejemplo: comprobar si las dependencias entre tareas forman un ciclo.

\begin{lstlisting}
bool hayCiclo = tieneCicloDirigido(n, adj);

cout << (hayCiclo ? "Dependencias invalidas" : "Orden posible");
\end{lstlisting}

\section{Aplicación 27: Kahn (BFS) --- Recomendado}
Ejemplo: obtener un orden válido para cursar materias con prerrequisitos.

\begin{lstlisting}
vector<int> orden = topoSortKahn(n, adj);

if (orden.empty())
    cout << "Hay un ciclo";
else
    for (int materia : orden) cout << materia << ' ';
\end{lstlisting}

\section{Aplicación 28: DFS (post-orden invertido)}
Ejemplo: construir un orden topológico cuando ya sabemos que el grafo es acíclico.

\begin{lstlisting}
vector<int> orden = topoSortDFS(n, adj);

for (int x : orden) cout << x << ' ';
\end{lstlisting}

\section{Aplicación 29: Dijkstra (pesos no negativos)}
Ejemplo: encontrar la ruta más barata entre ciudades cuando las carreteras tienen costos no negativos.

\begin{lstlisting}
vector<ll> dist = dijkstra(1, n, adjW);

cout << dist[n];              // costo minimo desde 1 hasta n
\end{lstlisting}

\section{Aplicación 30: Bellman-Ford (permite pesos negativos)}
Ejemplo: calcular rutas cuando existen descuentos que se representan con pesos negativos.

\begin{lstlisting}
auto [dist, cicloNegativo] =
    bellmanFord(1, n, edges);

if (cicloNegativo) cout << "Ciclo negativo";
else cout << dist[n];
\end{lstlisting}

\section{Aplicación 31: Floyd-Warshall (todos los pares)}
Ejemplo: obtener la distancia mínima entre cualquier par de ciudades.

\begin{lstlisting}
floydWarshall(dist, n);

cout << dist[2][5];            // mejor ruta 2 -> 5
\end{lstlisting}

\section{Aplicación 32: Kruskal (recomendado, más fácil de codear)}
Ejemplo: conectar todas las ciudades gastando el mínimo posible en carreteras.

\begin{lstlisting}
sort(edges.begin(), edges.end());
ll costo = kruskal(n, edges);

cout << costo;
\end{lstlisting}

\section{Aplicación 33: Prim (mejor para grafos densos)}
Ejemplo: construir una red mínima de cables conectando todos los edificios.

\begin{lstlisting}
ll costo = prim(n, adjW);
cout << costo;
\end{lstlisting}

\section{Aplicación 34: Componentes Fuertemente Conexas: Tarjan}
Ejemplo: encontrar grupos de páginas web donde cada página puede llegar a las demás.

\begin{lstlisting}
tarjanSCC(n);

cout << "La pagina 3 pertenece a SCC "
     << comp[3];
\end{lstlisting}

\section{Aplicación 35: Puentes y Puntos de Articulación}
Ejemplo: detectar carreteras críticas cuya eliminación desconecta una red.

\begin{lstlisting}
encontrarPuentesYArticulacion(n);

for (auto [u, v] : puentes)
    cout << u << " - " << v << '\\n';
\end{lstlisting}

\section{Aplicación 36: Ancestro Común Más Bajo (LCA) --- Binary Lifting}
Ejemplo: encontrar el ancestro común de dos empleados en un árbol jerárquico.

\begin{lstlisting}
lcaInit(n, 1);

int jefeComun = lcaQuery(12, 19);
cout << jefeComun;
\end{lstlisting}

\section{Aplicación 37: 2-SAT}
Ejemplo: elegir valores booleanos que satisfagan restricciones como (A o B).
...
if (sat.solve())
    cout << "Existe una asignacion";
else
    cout << "No existe";
\end{lstlisting}

\section{Aplicación 38: Flujo Máximo: Edmonds-Karp}
Ejemplo: calcular cuántas unidades de agua pueden enviarse desde una fuente hasta un destino.

\begin{lstlisting}
int maxFlow = edmondsKarp(origen, destino);
cout << maxFlow;
\end{lstlisting}

\section{Aplicación 39: Grafos Implícitos en Grid/Matriz}
Ejemplo: encontrar la distancia mínima entre dos casillas de un laberinto.
...
\section{Aplicación 40: KMP (Knuth-Morris-Pratt)}
Ejemplo: encontrar todas las posiciones donde aparece una palabra dentro de un texto.

\begin{lstlisting}
string texto = "abracadabra";
string patron = "abra";

vector<int> posiciones = kmp_search(texto, patron);
// posiciones = {0, 7}
\end{lstlisting}

\section{Aplicación 41: Z-Function}
Ejemplo: comparar rápidamente cada sufijo con el prefijo de una cadena.
...
\section{Aplicación 42: Hashing de Strings (Polinomial)}
Ejemplo: comparar rápidamente dos subcadenas para detectar si tienen el mismo hash.

\begin{lstlisting}
StringHash h("programacion");
...
\section{Aplicación 43: Manacher (Palíndromos)}
Ejemplo: encontrar el palíndromo más largo de una palabra.

\begin{lstlisting}
string s = "abacaba";
vector<int> radio = manacher(s);

int mejor = 0;
for (int r : radio) mejor = max(mejor, r - 1);
cout << mejor;
\end{lstlisting}

\section{Aplicación 44: Boyer-Moore (Búsqueda de Patrones)}
Ejemplo: buscar una palabra dentro de un texto usando saltos grandes cuando hay coincidencias parciales.
...
\section{Aplicación 45: Boyer-Moore (Búsqueda de Patrones) (ejemplo 2)}
Ejemplo directo de uso de mayoritario sobre datos pequeños para ver qué resultado produce.

\begin{lstlisting}
// Datos de ejemplo
// mayoritario(...);
cout << "Resultado obtenido por mayoritario";
\end{lstlisting}

\section{Aplicación 46: Plantilla General de DP (Top-Down / Memoización)}
Ejemplo: contar cuántos caminos existen en una cuadrícula moviéndose solo derecha/abajo.

\begin{lstlisting}
memset(visitadoDp, 0, sizeof(visitadoDp));

cout << solve(3, 4);           // cantidad de caminos
\end{lstlisting}

\section{Aplicación 47: Versión con Arreglo}
Ejemplo: calcular Fibonacci cuando el máximo N se conoce y cabe en un arreglo.
...
\section{Aplicación 48: Versión con unordered\_map}
Ejemplo directo de uso de fibonacci\_memo\_map sobre datos pequeños para ver qué resultado produce.

\begin{lstlisting}
// Datos de ejemplo
// fibonacci_memo_map(...);
cout << "Resultado obtenido por fibonacci_memo_map";
\end{lstlisting}

\section{Aplicación 49: Fast Doubling con Memoización}
Ejemplo: calcular Fibonacci para valores gigantes en O(log N).

\begin{lstlisting}
memoFastDouble.clear();
cout << fibonacci_fast_doubling(1000000000000LL);
\end{lstlisting}

\section{Aplicación 50: Mochila 0-1 (0-1 Knapsack)}
Ejemplo: elegir objetos de mayor valor sin superar el peso máximo de una mochila.

\begin{lstlisting}
vector<int> peso = {2, 3, 4};
vector<int> valor = {4, 5, 7};

cout << knapsack(5, peso, valor, peso.size());
\end{lstlisting}

\section{Aplicación 51: Longest Common Subsequence (LCS)}
Ejemplo: medir cuánto contenido mantienen en común dos secuencias.

\begin{lstlisting}
cout << lcs("ABCBDAB", "BDCABA");
\end{lstlisting}

\section{Aplicación 52: Longest Increasing Subsequence (LIS)}
Ejemplo: encontrar la mayor secuencia de precios que crece estrictamente.

\begin{lstlisting}
vector<int> precios = {10, 5, 8, 12, 3, 15};
cout << lis(precios);
\end{lstlisting}

\section{Aplicación 53: DP con Máscaras de Bits (Bitmask DP - TSP)}
Ejemplo: encontrar la ruta mínima que visita todas las ciudades y regresa al inicio.

\begin{lstlisting}
memset(dp, -1, sizeof(dp));
cout << tsp(1, 0);            // empieza en la ciudad 0
\end{lstlisting}

\section{Aplicación 54: DP en Árboles (Tree DP)}
Ejemplo: escoger un conjunto de empleados sin seleccionar simultáneamente a un jefe y su subordinado directo.

\begin{lstlisting}
tree_dp(1, -1);
cout << max(dpIncluye[1], dpExcluye[1]);
\end{lstlisting}

\section{Aplicación 55: Operaciones Básicas y Trucos Fundamentales}
Ejemplo: representar permisos de usuario usando bits.

\begin{lstlisting}
int permisos = 0;
permisos |= (1 << 2);          // activar permiso 2
bool puedeEditar = (permisos >> 2) & 1;
\end{lstlisting}

\section{Aplicación 56: Conteo de Bits (Popcount)}
Ejemplo: contar cuántos permisos están activos en una máscara.

\begin{lstlisting}
int permisos = 42;
cout << __builtin_popcount(permisos);
\end{lstlisting}

\section{Aplicación 57: Conteo de Bits (Popcount) (ejemplo 2)}
Ejemplo directo de uso de popcount\_table sobre datos pequeños para ver qué resultado produce.

\begin{lstlisting}
auto cnt = popcount_table(20);
cout << cnt[13];
\end{lstlisting}

\section{Aplicación 58: Suma y Resta Binaria (Bit a Bit)}
Ejemplo: realizar una suma sin utilizar directamente el operador +.

\begin{lstlisting}
cout << suma_bitwise(13, 29);
cout << resta_bitwise(29, 13);
\end{lstlisting}

\section{Aplicación 59: Multiplicación Binaria (Russian Peasant / Egyptian Multiplication)}
Ejemplo: multiplicar bajo un módulo muy grande evitando un producto directo peligroso.

\begin{lstlisting}
cout << mulmod_binario(123456789LL,
                         987654321LL,
                         1000000000000000003LL);
\end{lstlisting}

\section{Aplicación 60: Enumeración de Submáscaras (Submask Enumeration)}
Ejemplo: probar todos los subconjuntos de un conjunto de habilidades representado por una máscara.

\begin{lstlisting}
int habilidades = 0b1111;

for (int sub = habilidades; ; sub = (sub - 1) & habilidades) {
    cout << sub << '\\n';
    if (sub == 0) break;
}
\end{lstlisting}

\section{Aplicación 61: Propiedades y Trucos con XOR}
Ejemplo: encontrar el número que aparece una sola vez cuando todos los demás aparecen dos veces.

\begin{lstlisting}
vector<int> a = {4, 7, 2, 7, 4};
cout << unico_no_repetido(a);   // 2
\end{lstlisting}

\section{Aplicación 62: std::bitset: Uso y Conversión}
Ejemplo: representar un conjunto pequeño de opciones y hacer operaciones masivas sobre sus bits.

\begin{lstlisting}
bitset<8> permisos("10110010");
...
\section{Aplicación 63: std::bitset: Uso y Conversión (ejemplo 2)}
Consulta directamente el siguiente o anterior elemento en un conjunto ordenado.
...
\section{Aplicación 64: Casos de Uso de Bitmasking}
Ejemplo: subset sum acelerado: saber qué sumas pueden conseguirse con un conjunto de números.

\begin{lstlisting}
vector<int> pesos = {3, 5, 7};
auto alcanzables = subset_sum_alcanzable(pesos);

cout << alcanzables[10];
\end{lstlisting}

\section{Aplicación 65: Casos de Uso de Bitmasking (ejemplo 2)}
Ejemplo directo de uso de dp sobre datos pequeños para ver qué resultado produce.

\begin{lstlisting}
// Datos de ejemplo
// dp(...);
cout << "Resultado obtenido por dp";
\end{lstlisting}

\section{Aplicación 66: Casos de Uso de Bitmasking (ejemplo 3)}
Ejemplo directo de uso de subset\_sum\_alcanzable sobre datos pequeños para ver qué resultado produce.

\begin{lstlisting}
// Datos de ejemplo
// subset_sum_alcanzable(...);
cout << "Resultado obtenido por subset_sum_alcanzable";
\end{lstlisting}

\section{Aplicación 67: Casos de Uso de Bitmasking (ejemplo 4)}
Ejemplo directo de uso de DP sobre datos pequeños para ver qué resultado produce.
...
\section{Aplicación 68: MCD (Máximo Común Divisor) y MCM (Mínimo Común Múltiplo)}
Ejemplo: sincronizar dos eventos que se repiten cada a y b segundos.
...
\section{Aplicación 69: Algoritmo de Euclides Extendido}
Ejemplo: encontrar el inverso modular de a cuando gcd(a,m)=1.

\begin{lstlisting}
ll x, y;
ll g = extgcd(3, 11, x, y);
...
\section{Aplicación 70: Criba de Eratóstenes (Sieve)}
Ejemplo: precalcular primos y después factorizar muchos números rápidamente.

\begin{lstlisting}
sieve();

vector<int> factores = factorizar(360);
// 2 2 2 3 3 5
\end{lstlisting}

\section{Aplicación 71: Exponenciación Rápida (Fast/Modular Power)}
Ejemplo: calcular una potencia enorme módulo MOD.

\begin{lstlisting}
cout << modpow(2, 1000000000LL, 1000000007LL);
\end{lstlisting}

\section{Aplicación 72: Inverso Multiplicativo Modular}
Ejemplo: dividir bajo módulo primo usando el inverso modular.

\begin{lstlisting}
ll a = 3;
ll inv = inv_mod(a);

cout << inv;            // 333333336 para MOD = 1e9+7
\end{lstlisting}

\section{Aplicación 73: Combinatoria: Factoriales y nCr con Módulo}
Ejemplo: contar cuántas formas hay de elegir r personas de n.
...
\section{Aplicación 74: Test de Primalidad Miller-Rabin}
Ejemplo: comprobar rápidamente si un número grande es primo.

\begin{lstlisting}
ll n = 1000000007LL;
cout << (miller_rabin(n) ? "Primo" : "Compuesto");
\end{lstlisting}

\section{Aplicación 75: Teorema Chino del Resto (CRT) --- Implementación}
Ejemplo: encontrar un número que cumpla simultáneamente varios restos.
...
if (crt(r, m, resR, resM))
    cout << resR;       // 8
\end{lstlisting}

\section{Aplicación 76: Función de Möbius}
Ejemplo: precalcular mu(n) para técnicas de inclusión-exclusión sobre divisores.

\begin{lstlisting}
calcular_mobius();

cout << mu[6];          // 1, porque 6 = 2 * 3
\end{lstlisting}

\section{Aplicación 77: Fórmula de Legendre (Exponente de un Primo en $N!$)}
Ejemplo: saber cuántas veces aparece el factor 2 dentro de 100!.

\begin{lstlisting}
cout << legendre_exponente(100, 2);
\end{lstlisting}

\section{Aplicación 78: Primitivas Básicas: Punto, Producto Cruz y Punto}
Ejemplo: saber si tres puntos forman un giro a la izquierda o a la derecha.

\begin{lstlisting}
Point A{0, 0}, B{2, 0}, C{1, 3};

double area2 = cross(A, B, C);
// area2 > 0: C esta a la izquierda de A->B
\end{lstlisting}

\section{Aplicación 79: Envolvente Convexa (Convex Hull - Andrew Monotone Chain)}
Ejemplo: encontrar el contorno exterior de un conjunto de puntos GPS.
...
vector<Point> borde = convex_hull(puntos);
\end{lstlisting}

\section{Aplicación 80: Área y Punto Dentro de Polígono}
Ejemplo: calcular el área de una parcela y comprobar si una coordenada cae dentro.

\begin{lstlisting}
vector<Point> parcela = {
    {0,0}, {4,0}, {4,3}, {0,3}
};

cout << poly_area(parcela);
cout << point_in_polygon({2,1}, parcela);
\end{lstlisting}

\section{Aplicación 81: Intersección de Segmentos}
Ejemplo: comprobar si dos carreteras se cruzan.
...
\section{Aplicación 82: Punto de Intersección de Dos Rectas}
Ejemplo: encontrar exactamente dónde se cruzan dos trayectorias.

\begin{lstlisting}
Point out;
bool ok = line_intersection(
    {0,0}, {4,4},
    {0,4}, {4,0},
    out
);

if (ok) cout << out.x << ' ' << out.y;
\end{lstlisting}

\section{Aplicación 83: Distancia Punto-Recta y Punto-Segmento}
Ejemplo: calcular qué tan cerca está una persona de una carretera.

\begin{lstlisting}
Point persona{2, 3};
Point A{0,0}, B{5,0};

cout << dist_point_line(persona, A, B);
cout << dist_point_segment(persona, A, B);
\end{lstlisting}

\section{Aplicación 84: Círculos: Pertenencia e Intersección}
Ejemplo: comprobar si una ubicación está dentro de una zona de cobertura y si dos coberturas se intersectan.

\begin{lstlisting}
Circle a{{0,0}, 5};
Circle b{{6,0}, 4};

cout << point_in_circle({3,0}, a);

auto cortes = circle_circle_intersection(a, b);
\end{lstlisting}

\section{Aplicación 85: Par de Puntos Más Cercano (Closest Pair)}
Ejemplo: encontrar las dos antenas más cercanas entre muchas antenas.

\begin{lstlisting}
vector<Point> antenas = {
    {0,0}, {10,10}, {1,1}, {20,3}
};

cout << closest_pair(antenas);
\end{lstlisting}

\section{Aplicación 86: Rotating Calipers: Diámetro de un Conjunto de Puntos}
Ejemplo: encontrar la máxima distancia entre dos puntos de una ciudad.
...
cout << poly_diameter(puntos);
\end{lstlisting}

\section{Aplicación 87: Ordenamiento Angular (Radial Sort) y Convexidad}
Ejemplo: ordenar puntos alrededor de una antena para procesarlos en sentido angular.
...
sort_by_angle(puntos, centro);
cout << is_convex_polygon(puntos);
\end{lstlisting}

\section{Aplicación 88: Interpolación de Hermite (Cúbica)}
Ejemplo: generar una posición suave entre dos puntos con velocidades inicial y final conocidas.

\begin{lstlisting}
Point p0{0,0}, p1{10,5};
Point m0{5,0}, m1{5,5};

Point p = hermite_interp(p0, m0, p1, m1, 0.5);
\end{lstlisting}

\section{Aplicación 89: Interpolación de Hermite (Cúbica) (ejemplo 2)}
Ejemplo directo de uso de catmull\_rom\_tangents sobre datos pequeños para ver qué resultado produce.

\begin{lstlisting}
// Datos de ejemplo
// catmull_rom_tangents(...);
cout << "Resultado obtenido por catmull_rom_tangents";
\end{lstlisting}

\section{Aplicación 90: Spline Cúbico Natural}
Ejemplo: interpolar suavemente una serie de mediciones sin forzar cambios bruscos de pendiente.

\begin{lstlisting}
CubicSpline spline;
spline.build(x, y);

double estimado = spline.eval(2.5);
\end{lstlisting}

\section{Aplicación 91: Sumas y Series}
Ejemplo: calcular una suma hasta N sin recorrer todos los números.

\begin{lstlisting}
ll n = 1000000;
cout << suma_naturales(n);
cout << suma_impares(n);
\end{lstlisting}

\section{Aplicación 92: Combinatoria}
Ejemplo: contar permutaciones y combinaciones usando factoriales precalculados.
...
\section{Aplicación 93: Geometría}
Ejemplo: calcular el área de un triángulo a partir de sus lados o coordenadas.

\begin{lstlisting}
cout << area_triangulo_heron(3, 4, 5);

cout << area_triangulo_coords(
    {0,0}, {4,0}, {0,3}
);
\end{lstlisting}

\section{Aplicación 94: Probabilidad y Recurrencias}
Ejemplo: calcular la esperanza y varianza de la suma de variables aleatorias independientes.

\begin{lstlisting}
cout << esperanza_suma(valores);
cout << varianza(valores);
\end{lstlisting}

\section{Aplicación 95: Teorema Maestro (guía de referencia, no se codea)}
Ejemplo directo de uso de rapida sobre datos pequeños para ver qué resultado produce.

\begin{lstlisting}
// Datos de ejemplo
// rapida(...);
cout << "Resultado obtenido por rapida";
\end{lstlisting}

\section{Aplicación 96: Matrioshka}
Ejemplo: resolver el problema completo: comprobar si una secuencia de tamaños puede formar muñecas anidadas válidas.
...
vector<int> cajas(n);
for (int &x : cajas) cin >> x;

// El programa de la seccion aplica la pila
// para verificar si todas las Matrioshkas son validas.
\end{lstlisting}
