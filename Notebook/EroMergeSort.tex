...
% =========================================================================
% CAPÍTULO: CASTEOS Y CONVERSIONES
% =========================================================================
\chapter{Casteos y Conversiones}
Esta sección reúne conversiones frecuentes en programación competitiva.
Cada ejemplo es autocontenido y muestra tanto la conversión como su uso
práctico.

\section{Binario a Decimal}
La forma más sencilla es recorrer los bits como caracteres. También podemos
usar \texttt{bitset} cuando el tamaño sea fijo.
...
Podemos usar \texttt{bitset} para un tamaño fijo o construir la representación
manualmente.

\begin{lstlisting}[language=C++]
int n = 45;

string binario = bitset<8>(n).to_string();

cout << binario << '\n';
// Salida: 00101101
\end{lstlisting}

\section{String a Entero}
\texttt{stoi} convierte un \texttt{string} en \texttt{int}. Para valores
grandes podemos usar \texttt{stoll}.
...
\section{Entero a String}
\texttt{to\_string} convierte un número a texto.
...
\section{String a Long Long}
Útil cuando el número puede superar el rango de un \texttt{int}.
...
cout << x << '\n';
// Salida: 1000000000000
\end{lstlisting}

\section{Char a Entero}
Para obtener el valor numérico de un dígito usamos la resta con \texttt{'0'}.
...
\section{Entero a Char}
Si tenemos un dígito entre 0 y 9 podemos convertirlo usando \texttt{'0'}.
...
\section{Char a Mayúscula y Minúscula}
Las funciones de \texttt{cctype} permiten cambiar caracteres fácilmente.
...
\section{String a Mayúsculas}
Útil para normalizar palabras antes de compararlas.
...
\section{String a Minúsculas}
La misma idea puede utilizarse para convertir todo el texto a minúsculas.
...
\section{Decimal a Hexadecimal}
\texttt{stringstream} permite convertir fácilmente entre bases.
...
\section{Hexadecimal a Decimal}
Indicamos base 16 al usar \texttt{stoi}.
...
\section{Decimal a Cualquier Base}
Para una base entre 2 y 36 podemos construir la representación manualmente.

\begin{lstlisting}[language=C++]
string convertirBase(int n, int base) {
    const string digitos =
...
cout << convertirBase(45, 2) << '\n';
// Salida: 101101
\end{lstlisting}

\section{Cualquier Base a Decimal}
Podemos interpretar cada dígito con la fórmula posicional.
...
long long aDecimal(string s, int base) {
    long long ans = 0;

    for (char c : s)
        ans = ans * base + valor(c);

    return ans;
}

cout << aDecimal("101101", 2) << '\n';
// Salida: 45
\end{lstlisting}

\section{String a \texttt{double}}
Cuando necesitamos números con parte decimal podemos usar \texttt{stod}.
...
\section{Entero a String con Formato}
Cuando necesitamos controlar el formato, \texttt{stringstream} es útil.
...
\section{ASCII: Char a Código}
Cada carácter tiene un valor numérico ASCII.
...
\section{Código ASCII a Char}
También podemos hacer la conversión inversa.
...
\section{String Numérico a Vector de Dígitos}
Muy útil para problemas donde necesitamos procesar cada dígito.
...
\section{Vector de Dígitos a String}
La operación inversa permite reconstruir el número como texto.

\begin{lstlisting}[language=C++]
vector<int> digitos = {3, 1, 4, 1, 5, 9};
...
cout << "Primer indice >= x: " << (lb - v.begin()) << '\n';
cout << "Cantidad de x: " << (ub - lb) << '\n';
\end{lstlisting}
...
\begin{lstlisting}
vector<int> v = {1, 2, 2, 4, 7, 9};
cout << cnt_le(v, 4) << '\n';   // 4 elementos <= 4
cout << cnt_lt(v, 4) << '\n';   // 3 elementos < 4
cout << cnt_range(v, 2, 7) << '\n'; // 5 elementos en [2,7]
\end{lstlisting}
...
\end{lstlisting}
\subsubsection*{Ejemplo de uso}
Ejemplo: construimos un grafo pequeño usando pares {nodo, vecino}, ejecutamos BFS desde el nodo 0 y al final reconstruimos e imprimimos el recorrido hasta el nodo 5.
\begin{lstlisting}
pair<int,int> par1 = {0, 1};
pair<int,int> par2 = {0, 2};
pair<int,int> par3 = {1, 3};
pair<int,int> par4 = {2, 4};
pair<int,int> par5 = {3, 5};
pair<int,int> par6 = {4, 5};

vector<vector<int>> adj(6);
adj[par1.first].push_back(par1.second);
adj[par2.first].push_back(par2.second);
adj[par3.first].push_back(par3.second);
adj[par4.first].push_back(par4.second);
adj[par5.first].push_back(par5.second);
adj[par6.first].push_back(par6.second);

// Si el grafo es no dirigido, agregar tambien la vuelta.
adj[1].push_back(0);
adj[2].push_back(0);
adj[3].push_back(1);
adj[4].push_back(2);
...
vector<int> camino = reconstruirCamino(5);
for (int v : camino)
    cout << v << ' ';
...
auto tangentes = catmull_rom_tangents(p0, p1, p2);
cout << tangentes.first.x << ' ' << tangentes.first.y << '\n';
\end{lstlisting}
...
\begin{lstlisting}
// El Teorema Maestro no es una funcion para ejecutar:
// se usa para clasificar recurrencias como
// T(n) = aT(n/b) + f(n).
// Ejemplo: T(n)=2T(n/2)+O(n) -> O(n log n).
\end{lstlisting}
